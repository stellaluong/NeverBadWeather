...
namespace NeverBadWeather.ApplicationServices
{
    public class ClothingRecommendationService
    {
        ...
        public async Task<bool> CreateRule(ClothingRule rule)
        {
            var rowsAffected = await _clothingRuleRepository.Create(rule);
            return rowsAffected == 1;
        }
        public async Task<bool> UpdateRule(ClothingRule rule)
        {
